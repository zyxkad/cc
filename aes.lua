...
local function expandKey(key)
	...
	local nk = #key / 4
	for i = 1, nk do
		enc[i] = b2uint32({key:byte((i - 1) * 4 + 1, i * 4)})
	end
	for i = nk, kl - 1 do
		local t = enc[i]
		if i % nk == 0 then
...
function Cipher:encryptInts(src)
	assert(#src == 4)
	return encryptBlock(self._enc, src)
end

function Cipher:decryptInts(src)
	assert(#src == 4)
	return decryptBlock(self._dec, src)
end
...
function CBCStream:encryptInts(src)
	assert(#src % 4 == 0)
	local res = {}
	for i = 1, #src, 4 do
...
function CBCStream:decryptInts(src)
	assert(#src % 4 == 0)
	local res = {}
	for i = 1, #src, 4 do
...
function CFBStream:encryptInts(src)
	assert(#src % 4 == 0)
	local res = {}
	for i = 1, #src, 4 do
...
function CFBStream:decryptInts(src)
	assert(#src % 4 == 0)
	local res = {}
	for i = 1, #src, 4 do
		local s = {table.unpack(src, i, i + 3)}
...
function OFBStream:encryptInts(src)
	assert(#src % 4 == 0)
	local res = {}
	for i = 1, #src, 4 do
...
function OFBStream:decryptInts(src)
	assert(#src % 4 == 0)
	local res = {}
	for i = 1, #src, 4 do
